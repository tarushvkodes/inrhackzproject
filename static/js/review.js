/**
 * Socratic — Session Review Logic
 * Displays session summary, learning insights, and conversation replay.
 */
...
function setupActions() {
    document.getElementById('deleteSession').addEventListener('click', async () => {
        ...
        try {
            const res = await fetch(`/api/session/${SESSION_ID}`, { method: 'DELETE' });
...
async function loadReview() {
    try {
        ...
        document.getElementById('reviewDuration').textContent = session.duration_minutes + 'm';
        document.getElementById('reviewDifficulty').textContent = capitalize(session.highest_difficulty);
        document.getElementById('reviewHints').textContent = session.hints_used;
        ...
        if (summary) {
            ...
            if (summary.recommended_next_topics && summary.recommended_next_topics.length > 0) {
                nextDiv.innerHTML = summary.recommended_next_topics
                    .map(t => `<a class="next-topic-tag" href="/">${escapeHtml(t)}</a>`).join('');
            } else {
        ...
        // Show the last unanswered question if any
        ...
        document.getElementById('summaryContent').innerHTML =
            '<p style="color:var(--danger);">Failed to load session data.</p>';
    }
}
